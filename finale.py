...
'''
def actualizar_posicion_jugador( x_cuadricula, y_cuadricula):
    print(f"Actualizando posición: x={x_cuadricula}, y={y_cuadricula}")
    connection = conectar_db()
    if connection is None:
        print("Connection to database failed.")
        return
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(f"""
                UPDATE Jugador 
                SET x = {x_cuadricula}, y = {y_cuadricula}
                WHERE IdJugador = 1  -- Asegúrate de que este ID sea el correcto
            """)
            connection.commit()
            print("Actualización exitosa de la posición")
    except pymysql.MySQLError as e:
...
def mover_carro_global(letra):
    global car_x, car_y

    # Mover el carro basado en la tecla ganadora
    ...
    # Guardar la nueva posición global del carro en la base de datos
    connection = conectar_db()
    if connection:
        try:
            with connection.cursor() as cursor:
                cursor.execute("""
                    UPDATE Jugador SET x = %s, y = %s WHERE IdJugador = 1
                """, (car_x // grid_size + 1, car_y // grid_size + 1))
                connection.commit()
        except pymysql.MySQLError as e:
            print(f"Error actualizando posición del carro: {e}")
        finally:
            connection.close()

def obtener_posicion_carro():
    global car_x, car_y
    ...
    if connection:
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT x, y FROM Jugador WHERE IdJugador = 1")
                result = cursor.fetchone()
                if result:
                    # Actualizar la posición del carro globalmente
                    car_x = (result[0] - 1) * grid_size
                    car_y = (result[1] - 1) * grid_size
        except pymysql.MySQLError as e:
            print(f"Error obteniendo posición del carro: {e}")
        finally:
            connection.close()
...
def actualizar_posiciones_de_jugadores():
    connection = conectar_db()
    if connection is None:
        print("Connection to database failed.")
        return None
    ...
    try:
        with connection.cursor() as cursor:
            ...
            for jugador in jugadores:
                jugador_id, pos_x, pos_y = jugador
                # Actualizar la posición de los carros en el juego (ajustar a grid_size)
                # Puedes distinguir entre jugadores usando colores o etiquetas
                dibujar_carro(pos_x * grid_size, pos_y * grid_size, jugador_id)
    except pymysql.MySQLError as e:
        print(f"Error al obtener posiciones de jugadores: {e}")
    finally:
        connection.close()

def dibujar_carro(pos_x, pos_y, jugador_id):
    # Puedes usar diferentes colores o etiquetas para distinguir los carros
    pygame.draw.rect(screen, RED, (pos_x, pos_y, grid_size, grid_size))
...
def juego(user_id, id_jugador, tecla_ganadora_orden):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(f"SELECT x, y FROM Jugador WHERE IdJugador = 1")
            result = cursor.fetchone()
            if result:
                car_x = (result[0] - 1) * grid_size  # Ajustar según la cuadrícula
                car_y = (result[1] - 1) * grid_size  # Ajustar según la cuadrícula
    ...
    votacion_terminada = False  # Nueva variable para controlar la votación

    manager = pygame_gui.UIManager((900, 800))
    dibujar_botones(manager)

    back_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((750, 10), (140, 50)),
        text='Regresar',
        manager=manager
    )
    ...
    filas, columnas = 10, 8  # Cambiarlo si es necesario
    matriz = crear_matriz(filas, columnas)
    ...
    while running:
        ...
        dibujar_cuadricula()  # Dibujar la cuadrícula
            ...
            # Obtener y actualizar las posiciones de todos los jugadores desde la base de datos
            # Obtener la posición global del carro en cada iteración para que esté sincronizado
        obtener_posicion_carro()

        # Dibujar el carro del jugador actual (local)
        ...
        # Verificar si el tiempo de votación ha terminado, pero solo una vez
        if (time.time() - tiempo_inicio_votacion) >= (remaining_time):
            votacion_terminada = True  # Marcar que la votación terminó
                    ...
                    try:
                        with connection.cursor() as cursor:
                            ...
                            if result:
                                ...
                                mover_carro_global(tecla_ganadora)
                            else:
        ...
        # Enviar posición del carro a la base de datos cada 5 segundos
        if tiempo_envio >= 5:
            print("Enviando posición del carro a la base de datos")
            imprimir_posicion_carro()  # Enviar la posición del carro rojo a la base de datos
